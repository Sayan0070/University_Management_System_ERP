...
<body>
  ...
  if(isset($_POST['Filter']))
  {
          ...
          if($res = $con->query($sql)){ 
            if($res->num_rows > 0){ 
              while($row = $res->fetch_array()){ 
                  $sql2="UPDATE `marks` SET `UserName`='".$row['UserName']."',`CA1`='".$_POST["CA1-".$row['StudentID']]."',
                  `CA2`='".$_POST["CA2-".$row['StudentID']]."',`CA3`='".$_POST["CA3-".$row['StudentID']]."',`CA4`='".$_POST["CA4-".$row['StudentID']]."' WHERE UserName='".$row['UserName']."'";                    
    ...
    <li class="nav-item">
      ...
      <li>
        <a href="/EditMarks.php" class="nav-link active">
        ...
        if($res->num_rows > 0){ 
            echo "<table class='table table-striped mt-1'>"; 
                echo "<tr>"; 
                    ...
                    echo "<th colspan='3' scope='col' style='text-align:center'>CA1 </th>";
                    echo "<th colspan='3' scope='col' style='text-align:center'>CA2 </th>";
                    echo "<th colspan='3' scope='col' style='text-align:center'>CA3 </th>";
                    echo "<th colspan='3' scope='col' style='text-align:center'>CA4 </th>"; 
                echo "</tr>"; 
            ...
            while($row = $res->fetch_array()){ 
                echo "<tr>"; 
                    ...
                    $sql2 = "SELECT * FROM marks where UserName='".$row['UserName']."'";
                    $req2 = mysqli_query($con, $sql2);
                    $res2 = $con->query($sql2);
                    while($row2 = $res2->fetch_array()){
                        echo "<td colspan='3' >" . 
                        "<div class='form-outline'>
                          <input type='number' id='typeNumber' class='form-control' name='CA1-".$row['StudentID']."' value='".$row2['CA1']."' />
                        </div>"."</td>";
                        echo "<td colspan='3' >" . 
                        "<div class='form-outline'>
                          <input type='number' id='typeNumber' class='form-control' name='CA2-".$row['StudentID']."' value='".$row2['CA2']."'/>
                        </div>"."</td>";
                        echo "<td colspan='3' >" . 
                        "<div class='form-outline'>
                          <input type='number' id='typeNumber' class='form-control' name='CA3-".$row['StudentID']."' value='".$row2['CA3']."'/>
                        </div>"."</td>";
                        echo "<td colspan='3' >" . 
                        "<div class='form-outline'>
                          <input type='number' id='typeNumber' class='form-control' name='CA4-".$row['StudentID']."' value='".$row2['CA4']."'/>
                        </div>"."</td>";
                    }
