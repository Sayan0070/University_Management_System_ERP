        ...
        if($_POST['confirm']=='Confirm'){
            ...
            if($username_avail==0){
              echo "<script type='text/javascript'> alert('No user with this id found!'); </script> ";
                ...
                $sql3="SELECT * FROM books where status='Unavailable' and BookId='".$_POST['BookId']."'";
                $res2 = mysqli_query($con, $sql3);
                
                if($res2->num_rows>0){
                  $row = $res2->fetch_array();
                  $sql4="UPDATE `books` SET `UserName`='',`IssueDate`='',
                  `ReturnDate`='',`Status`='Available' WHERE Status='Unavailable' and BookId='".$_POST['BookId']."'";
...
?>
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      ...
      <li>
        <a href="/ReturnBook.php" class="nav-link active">
