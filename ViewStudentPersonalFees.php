...
<body>
    ...
    <a href="/" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto link-dark text-decoration-none">
      ...
      <span class="fs-4">Student Section</span>
    </a>
    <hr>
    <ul class="nav nav-pills flex-column mb-auto">
      ...
      <li>
        <a href="/ViewStudentPersonalFees.php" class="nav-link active">
    ...
    if($res = $con->query($sql)){ 
        ...
        $sql = "SELECT * FROM fees where UserName='".$_SESSION['Susername']."'";
        $res = mysqli_query($con, $sql);
        if($res = $con->query($sql)){ 
        if($res->num_rows > 0){ 
            echo "<table class='table table-striped mt-1'>"; 
                echo "<tr>";
                    echo "<th colspan='3' scope='col'>1st Year</th>"; 
                    echo "<th colspan='3' scope='col'>2nd Year</th>";
                    echo "<th colspan='3' scope='col'>3rd Year</th>"; 
                    echo "<th colspan='3' scope='col'>4th Year</th>";

                echo "</tr>"; 
                
            while($row = $res->fetch_array()){ 
                echo "<tr>"; 
                    echo "<td colspan='3' >" . $row['1stYear'] . "</td>"; 
                    echo "<td colspan='3' >" . $row['2ndYear'] . "</td>";
                    echo "<td colspan='3' >" . $row['3rdYear'] . "</td>";
                    echo "<td colspan='3' >" . $row['4thYear'] . "</td>";
